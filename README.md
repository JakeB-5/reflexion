...
Self-Generation은 Claude Code Hooks API를 통해 사용자의 프롬프트, 도구 사용, 에러를 수집하고, AI(`claude --print`)로 패턴을 분석하여 반복적인 작업을 자동화하는 데 필요한 커스텀 지침을 생성합니다.

- **대상 환경**: Vanilla Claude Code (플러그인 불필요)
- **배포 상태**: Phase 1~5 완료 (237개 테스트 통과)
- **핵심 특징**: 이중 계층 아키텍처 (배치 분석 + 실시간 보조)

## 주요 기능

- **자동 데이터 수집** — 프롬프트, 도구 사용 기록, 에러 로그를 8개 hook으로 실시간 수집
- **AI 패턴 분석** — `claude --print`로 반복 패턴 감지 (도구 시퀀스, 에러 패턴, 프롬프트 스타일)
...
├── lib/
│   ├── db.mjs                    # SQLite DB 관리
│   ├── ai-analyzer.mjs           # claude --print 래퍼
...
| **분석** | claude CLI | latest | `claude --print` (AI 패턴 분석) |

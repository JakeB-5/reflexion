...
### 결정 1: 이중 기록 (prompt-log + subagent-stats)

**근거:** prompt-log.jsonl은 전체 이벤트 로그(배치 분석용), subagent-stats.jsonl은 에이전트별 성능 통계(pre-tool-guide 소비용)로 역할 분리.

**대안 검토:**
- prompt-log만 사용: pre-tool-guide에서 전체 로그 스캔 필요, 성능 비효율
- subagent-stats만 사용: 배치 분석에서 서브에이전트 이벤트 누락
...
- [ ] stdin 읽기 + SubagentStopEntry를 prompt-log.jsonl에 append
- [ ] 기본 동작 테스트 (정상 입력, 비정상 입력)

### Phase 2: 성능 통계 집계

subagent-stats.jsonl에 통계 엔트리 기록

**산출물:**
- [ ] SubagentStatsEntry 스키마 정의 및 append 로직
- [ ] 성공/실패 판정 로직 구현
- [ ] 통계 기록 테스트
...
- [ ] pre-tool-guide에서 subagent-stats.jsonl 소비 테스트
- [ ] 훅 등록 JSON 검증
...
| subagent-stats.jsonl 무한 증가 | 🟢 LOW | 배치 분석에서 주기적 정리; pre-tool-guide는 최근 20건만 사용 |
...
- 이벤트 기록: prompt-log.jsonl append 확인
- 통계 기록: subagent-stats.jsonl append 확인
- exit 코드: 모든 경우 0

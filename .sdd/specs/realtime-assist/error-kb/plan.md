...
### 결정 1: 동기 I/O 사용 (readFileSync/appendFileSync)

**근거:** 훅 환경에서 프로세스가 즉시 종료되므로, 비동기 I/O가 완료 전에 프로세스가 끝날 위험이 있다. 동기 I/O로 기록 완료를 보장한다.

**대안 검토:**
- async/await + writeFile: 프로세스 종료 전 기록 누락 위험
...
| 동시 쓰기 시 JSONL 손상 | 🟡 MEDIUM | 단일 프로세스 실행 환경이므로 실질적 위험 낮음 |
